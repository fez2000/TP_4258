...
<md-empty-state
    ...
    :class="color"
    md-icon="shop"
    :md-label="$t(`projects.projetInProcess`)"
    ...
    <md-button @click="goToCreateProject()" class="md-primary md-raised">{{$t("projects.create_button2")}}</md-button>
      ...
      <v-row v-else-if="haveproject">
        ...
        <v-col  :sm="6" :md="4" :lg="4" :xl="3"  v-for="(i,index) in projects"  v-show="!i.deleted" :key="i._id" >
            <project-card :i="i" view="State" :cardWidth="cardWidth" :voter="voter" :darkTheme="darkTheme"  :cathegoriesList="cathegoriesList" ></project-card>
...
export default {
  ...
  methods: {
    ...
    getMore(){
      this.$axios.get('/api/project/findallnext')
      .then(({data})=>{
    ...
    getMyProject() {
      this.$axios.get('/api/project/findallstart')
      .then(({data})=>{
