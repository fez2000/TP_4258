            ...
            >
              <v-list-item dense>
                ...
                <v-list-item-icon v-if="event.document && event.document.cathegorie != 'image'">
                  ...
                  <v-icon
                    v-if="event.document && event.document.cathegorie == 'video'"
              ...
              <v-card-actions>
                ...
                <v-btn @click="comment()" class="col" text v-if="event.type == 'post' || event.type == 'doc'">
              ...
              <v-card-text>
                <v-text-field dense color="primary" filled rounded outlined append-outer-icon="mdi-send" placeholder="un commntaire"/>
              </v-card-text>
            </v-card>
          </div>
        </template>
...
export default {
  ...
  beforeDestroy() {
    if (secureSocket) {
      secureSocket.removeListener('updatedEvent', this.updatedEvent.bind(this))
  ...
  mounted() {
    ...
    secureSocket.on('updatedEvent', this.updatedEvent.bind(this))
    secureSocket.on("newEvent", this.waitEvent.bind(this));
    secureSocket.on("likeStatus", this.likeStatus.bind(this));
    secureSocket.on("likeOf", this.likeOf.bind(this));
    secureSocket.on("synlike", this.synlike.bind(this));
    secureSocket.on("tags", this.waitTags.bind(this));
    secureSocket.on("deletedEvent", this.localeDeleteEvent.bind(this));
    secureSocket.emit("getag");
    let url = new URL(window.location.href);
    if (url.searchParams.startAt) {
      this.fetchEvent("/api/event/startAt?id=" + url.searchParams.startAt);
    } else {
      this.fetchEvent();
    }
  },
  ...
  methods: {
      ...
      this.$axios
        ...
        .then(ev => {
          ...
          if (ev.data.status) {
            ...
            secureSocket.emit("checkLikeStatus", {
              id: item._id,
              myId: this.voter._id
            });
    ...
    deleteEvent() {
      ...
      this.$axios
        .delete("/api/event/" + this.eventToDelete._id)
        .then(({ data }) => {
          if (data.status) {
    ...
    fetchEvent(url = "/api/event/start") {
      this.isLoading = true;
      setTimeout(() => {
        this.$axios
          .get(url)
