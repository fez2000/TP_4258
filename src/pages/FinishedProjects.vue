    ...
    <div class="wrapper">
        <parallax class="section page-header header-filter" :style="headerStyle">
            <div class="container">
                <div class="md-layout">
                    ...
                    >
                        <h1 class="title"><span v-text="nb"></span> project(s)</h1>
                        <h4>
                        {{$t('finishedprojet.title')}}
                        </h4>
                        <br />
                        <md-button
                        @click="donate"
                        ...
                        target="_blank"
                        > <i style="font-size: 2.1rem!important;right: 10px;position: relative;" class=" mdi v-icon notranslate mdi-charity"></i> {{$t("donate.title")}}</md-button>
        ...
        <div class="main main-raised">
            <div class="section">
                <div class="container">
                    <div class="md-layout">
                        ...
                        <p class="description" style="text-align:justify">{{$t('donate.description1')}}<strong>{{$t('donate.slogan')}}</strong></p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="section text-center">
                ...
                <div class="team">
                    <div class="md-layout">
                        <div v-for="project of projects" :key="project._id" class="md-layout-item md-medium-size-33 md-small-size-100">
                            <div class="team-player">
                                <md-card md-with-hover>
                                    ...
                                    <small class="card-description text-muted"
                                        >{{project.short_description}}</small
                                    >
                                    </h4>

                                    <md-card-content>
                                        <p class="card-description">
                                            {{ (project.description)?(project.description.length > 200)?project.description.substr(0,150)+'...':project.description:''}}
                                        </p>
                                        <p class="card-description">
                                    ...
                                    <md-card-actions   style="justify-content: center;" class="text-center">
                                       <router-link :to="'/project/'+project.url">
                                        ...
                                        <md-tooltip md-direction="bottom">{{(project.docs.length > 0)? project.docs.length+' document(s)':'pas de document'}}</md-tooltip>
                                       </router-link>
                                       <v-badge
                                       ...
                                       <router-link v-if="project.voter" :to="'/in/'+project.voter.url">
                                        ...
                                        <md-tooltip md-direction="bottom">Initiateur {{project.voter.name.substr(0,50)}}</md-tooltip>
                                       </router-link>
...
head: {
    ...
    meta: [
        ...
        { name: "twitter:description",  content: "pages des projects fini" },
        { name: "twitter:image", content: "/assets/img/apple-icon.png" },
        { property: 'fb:app_id', content: '123456789' },
        { property: 'og:title', content: `Finished projects | ${process.env.APP_NAME}` },
        { property: "og:site_name", content: `${process.env.APP_NAME}` },
        { property: "og:url", content: `${process.env.BASE_URL||''}projects/finished` },
    ...
    link: [
        { rel: 'canonical', href: `${process.env.BASE_URL||''}projects/finished`, id: 'canonical' },
    ...
    methods: {
       ...
       fetchNb(){
            this.$axios.get('/api/project/filter/end/nb')
            .then((rep)=>{
                if (rep.data.status) {
                this.nb = rep.data.nb;
                }
        ...
        getvoterstart(){
            this.$axios.get('/api/project/filter/end/findallstart')
            .then(({ data })=>{
                if(data.status){
                ...
                this.$root.$emit('neterror', { err: err, callback: this.getvoterstart});
            })
        },
        getvoternext(){
            this.$axios.get('/api/project/filter/end/findallnext')
            .then(({ data })=>{
                if(data.status){
