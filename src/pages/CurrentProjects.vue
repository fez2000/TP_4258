...
head: {
    ...
    meta: [
        ...
        { name: "description", content: "pages des projects courant" },
        ...
        { property: 'og:title', content: `Current projects | ${process.env.APP_NAME}` },
        { property: "og:site_name", content: `${process.env.APP_NAME}` },
        { property: "og:url", content: `${process.env.BASE_URL||''}projects/current` },
    ...
    link: [
        { rel: 'canonical', href: `${process.env.BASE_URL||''}projects/current`, id: 'canonical' },
    ...
    methods: {
       fetchNb(){
            this.$axios.get('/api/project/filter/start/nb')
            .then((rep)=>{
                if (rep.data.status) {
                this.nb = rep.data.nb;
                }
        ...
        getvoterstart(){
            this.$axios.get('/api/project/filter/start/findallstart')
            .then(({ data })=>{
                if(data.status){
        ...
        getvoternext(){
            this.$axios.get('/api/project/filter/start/findallnext')
            .then(({ data })=>{
                if(data.status){
